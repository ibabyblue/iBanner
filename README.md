...
iOS 17+ 通用 Banner 无限轮播组件，UIKit 三页复用内核，SwiftUI 对外接口，零三方依赖。
...
```swift
dependencies: [
    .package(url: "https://github.com/your-org/iBanner", from: "1.0.0")
]
```
...
| `items` 运行时替换 | 重建三页，`currentIndex` 归零 |
...
- 内核为 UIScrollView 三页复用（宽度 3×），滚动结束后静默归位中心，实现无限轮播
- 对外接口完全 SwiftUI 化，使用方感知不到 UIKit
- 图片加载使用系统 `AsyncImage`，零三方依赖
- Capsule 指示器通过 `@Binding scrollProgress` 实现逐帧跟手动画
